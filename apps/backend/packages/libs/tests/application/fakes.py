...
class FakeDatabase:
    ...
    charges: dict[UUID, ChargeRow] = field(default_factory=dict)

    # 「書き換える前に行をロックしたか」を検証するための記録。
    locked_reservation_ids: list[UUID] = field(default_factory=list)
    locked_charge_ids: list[UUID] = field(default_factory=list)
...
class FakeBillingRepository:
    ...
    @staticmethod
    def _to_charge(row: ChargeRow) -> Charge:
    ...
    def create_charge(
        ...
        self._db.charges[reservation_id] = row
        return self._to_charge(row)

    def find_charge(self, reservation_id: UUID) -> Charge | None:
        row = self._db.charges.get(reservation_id)
        return self._to_charge(row) if row is not None else None

    def lock_charge(self, reservation_id: UUID) -> Charge | None:
        self._db.locked_charge_ids.append(reservation_id)
        return self.find_charge(reservation_id)

    def mark_charge_paid(self, reservation_id: UUID) -> Charge:
        row = self._db.charges[reservation_id]
        row.paid = True
        return self._to_charge(row)
