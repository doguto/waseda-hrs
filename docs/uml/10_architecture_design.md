...
### 5.1 ドメイン分析（01）の概念のうち、システム化しないもの

ドメイン分析（`01_domain_class_diagram.puml`）は**問題領域に存在する概念**のモデルであり、
システム化の範囲を表す図ではない。したがって 01 の概念がすべて実装に現れるわけではない。
実装と一対一で対応するのは**システム分析（09）**であり、01 と 09 の差分は下表のとおり
意図的なものである。

| 01 の概念 | 09・実装での扱い | 理由 |
|---|---|---|
| 利用者 | `Guest` エンティティ（`guests` テーブル） | 予約者の記録として必要 |
| 予約 | `Reservation`（`reservations`） | — |
| 客室 | `Room`（`rooms`） | — |
| 宿泊料金 | `RoomRate` / `ServiceUsage` / `Charge` | 「単価」「追加料金」「発行された請求」に分けて置いた |
| **ホテル** | **持たない** | 単一ホテルの運用を前提としたため、暗黙の1インスタンスとして扱う。複数ホテルを扱う要求が出た時点で `Hotel` を導入し、`rooms` に外部キーを足す拡張になる |
| **フロント係** | **持たない**（ユースケース図のアクタ） | システムの外部に立つ人であり、システム内部のオブジェクトではない。09にも「アクターはクラスとして扱わない」と明記している。担当者の記録や認証を要求に含めていないため、`staff` テーブルも設けていない |
| **チェックイン / チェックアウト** | **独立したエンティティは持たない**（`Reservation.status` の遷移として実現） | 業務上は「出来事」だが、システムが保持すべきは予約の現在状態であり、出来事そのものを実体として残す要求がない。履歴の参照が要求に加わればイベントの実体化（`check_ins` テーブル等）が必要になる |

**この差分が意味すること**: フロント係の担当記録・複数ホテル対応・宿泊履歴の3つは、
要求が追加された時点でドメインモデル側の変更なしに実装を足せる。ドメイン分析で概念として
既に洗い出してあるためである。逆にこれらを「今は作らない」と決めた判断は要求分析の結果で
あって、モデルの誤りではない。
